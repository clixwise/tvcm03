...
if TYPE_CHECKING:
    from c02_anom_01_grph_plot_ import PlotTranANOM_01_boxx
    from c02_anom_01_grph_plot_ import PlotConfANOM_01_boxx
...
# Timepoint outcomes
# ----
def exec_plot_boxx(plot_tran_pure: PlotTranANOM_01_boxx) -> None:
    # from c02_qol_01_grph_plot_ import PlotTranANOM_01_boxx
    
    trac = True
    plot_tran_conf:PlotConfANOM_01_boxx = plot_tran_pure.conf
    ...
    '''
    # 1. Define the levels for the MultiIndex
    groups = ['A', 'M', 'F']
    metrics = ['median', 'mean', 'age']

    # 2. Create the MultiIndex object
    columns = pd.MultiIndex.from_product([groups, metrics])

    # 3. Create mock data
    # We'll use 4 rows as seen in your info() output
    data = {
        ('A', 'median'): [25.5, 30.2, 22.1, 45.0],
        ('A', 'mean'):   [26.1, 31.0, 23.5, 44.8],
        ('A', 'age'):    ['18, 25', '30', '20, 22, 24', '40, 50'], # String objects
        
        ('M', 'median'): [28.0, 32.5, 21.0, 42.0],
        ('M', 'mean'):   [28.5, 33.0, 21.5, 41.5],
        ('M', 'age'):    [[18, 25], [32], [21], [42]],            # List objects
        
        ('F', 'median'): [24.0, 29.0, 23.0, 46.0],
        ('F', 'mean'):   [24.5, 29.5, 23.2, 45.5],
        ('F', 'age'):    ['Under 20', '30-40', '20s', 'Senior']   # Categorical strings
    }
