...
def main_exec_qol_01_stat(procTran:ProcTranStatQOL01, inpuTran:InpuTran, oupuTran:OupuTranFile):
    ...
    # Stat.prec
    # ----
    def proc_fram_post(statTran:StatTranQOL_01, framTran:FramTran):
        
        # Exec
        # ----
        df_fram = framTran.fram.copy()
        df_fram = df_fram[['workbook', 'patient_id', 'timepoint', 'none_t', 'mean_t', 'iter_t', 'gemi_t', 'copi_t', 'pati_isok', 'pati_50pc']]
        df_fram['VEINES_QOL_t'] = df_fram['copi_t'] # df_fram.rename(columns={'copi_t': 'VEINES_QOL_t'}, inplace=True)
    ...
    # Stat.Exec
    # ----
    def proc_stat_exec(statTran:StatTranQOL_01):
        statTran.stra = 'a'
        statTran.upda()
        
    # Stat.Oupu
    # ----
    def proc_stat_oupu(statTran:StatTranQOL_01, oupuTran:OupuTranFile): 
        oupuTran.fram_dict[StatTranQOL_01_desc.__name__] =  { "resu_publ" : { 'df':statTran.stat_tran_desc.resu_publ, 'mode':'md' }, "resu_tech" : { 'df':statTran.stat_tran_desc.resu_tech, 'mode':'md' } }
        oupuTran.fram_dict[StatTranQOL_01_mean.__name__] =  { "resu_plot" : { 'df':statTran.stat_tran_mean.resu_plot, 'mode':'md' } }
        oupuTran.fram_dict[StatTranQOL_01_mixd.__name__] =  { "resu_glob" : { 'df':statTran.stat_tran_mixd.resu_glob, 'mode':'md' }, "resu_deta" : { 'df':statTran.stat_tran_mixd.resu_deta, 'mode':'md' } }
        oupuTran.upda()
        oupuTran.fram_dict = {}
    ...
    statTran = StatTranQOL_01(procTran)
