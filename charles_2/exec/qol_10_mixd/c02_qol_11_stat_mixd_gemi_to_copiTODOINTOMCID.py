...
# https://copilot.microsoft.com/shares/hgG5AgXmUCTaTkG6A8HW9 
# https://copilot.microsoft.com/shares/2iJd9McvxN9qvw6zimzns 
# https://copilot.microsoft.com/shares/LbyUNw7ZRY75pyM5BftqC
#
def exec_stat_mixd_gemi_to_copi(stat_tran_adat: StatTranQOL_11_mixd, df_lon1:DataFrame) -> None:
    ...
    # ==== 
    model = smf.mixedlm("VEINES_QOL_t ~ C(timepoint)", df_modl, groups=df_fram["patient_id"])
    result = model.fit(reml=True, method="powell")
    print(result.summary())
    pass
    
    # $$$$
    # Exec 1 : Compute Distribution-based MCID threshold
    # $$$$
    def calculate_distribution_based_mcid(df: pd.DataFrame, lmm_result) -> pd.DataFrame:
        """
        Distribution-based MCID metrics for VEINES-QOL using variance components
        from the longitudinal mixed model.

        - Baseline SD (T0)
        - Residual SD (within-patient)
        - ICC (reliability)
        - SEM (Standard Error of Measurement)
        - SDC_95 (Smallest Detectable Change, 95% level)
        - 0.3 SD and 0.5 SD benchmarks
        """

        # 1. Baseline SD (T0)
        baseline_scores = df[df['timepoint'] == "T0"]["VEINES_QOL_t"]
        sd_baseline = baseline_scores.std(ddof=1)
        # 2. Variance components from LMM
        var_between = lmm_result.cov_re.iloc[0, 0]   # random intercept variance
        var_within = lmm_result.scale                # residual variance
        sd_within = np.sqrt(var_within)
        # 3. ICC (reliability)
        icc = var_between / (var_between + var_within)
        # 4. SEM (Standard Error of Measurement)
        sem = sd_baseline * np.sqrt(1 - icc)
        # 5. SDC (Smallest Detectable Change, 95% CI)
        sdc_95 = 1.96 * np.sqrt(2) * sem
        # 6. Build MCID table
        mcid_rows = [
            {"Metric": "baseline_SD", "Value": sd_baseline},
            {"Metric": "residual_SD", "Value": sd_within},
            {"Metric": "ICC_Reliability", "Value": icc},
            {"Metric": "SEM", "Value": sem},
            {"Metric": "SDC_95", "Value": sdc_95},
            {"Metric": "0.3_SD", "Value": 0.3 * sd_baseline},
            {"Metric": "0.5_SD", "Value": 0.5 * sd_baseline},
        ]
        df_mcid = pd.DataFrame(mcid_rows, columns=['Metric', 'Value'])
        print_yes(df_mcid, labl="df_mcid")
        return df_mcid
    #
    df_mcid = calculate_distribution_based_mcid(df_modl, result)
    mcid_thre = df_mcid.loc[df_mcid['Metric'] == '0.5_SD', 'Value'].values[0]
    pass

    # $$$$
    # Exec 2 : Population‑Level Change (Mean Change + CI)
    # $$$$
    def extract_population_changes(lmm_result):
        """
        Extract population-level mean changes from the LMM:
        - Mean change vs T0
        - Standard error
        - 95% CI
        - p-value
        ...
        for tp in timepoints:
            coef_name = f"C(timepoint)[T.{tp}]"

            rows.append({
                "Comparison": f"{tp}_vs_{baseline_label}",
                "mean_change": params[coef_name],
        ...
        df_popu_delt = pd.DataFrame(rows)
    ...
    # $$$$
    # Exec 3 : Cohen's d effect sizes for each timepoint comparison vs T0
    # $$$$
    def calculate_effect_sizes(df: pd.DataFrame) -> pd.DataFrame:
        """
        Calculate Cohen's d effect sizes for each timepoint comparison vs T0.
        Uses pooled SD from baseline and follow-up distributions.
        """

        baseline_scores = df[df['timepoint'] == "T0"]["VEINES_QOL_t"]
        baseline_sd = baseline_scores.std(ddof=1)
        ...
        for tp in timepoints:
            ...
            tp_sd = tp_scores.std(ddof=1)

            # Pooled SD
            pooled_sd = np.sqrt((baseline_sd**2 + tp_sd**2) / 2)
            ...
            rows.append({
                "Comparison": f"{tp}_vs_T0",
                "cohens_d": d,
                "Magnitude": magnitude
            })

        df_eff = pd.DataFrame(rows)
        print_yes(df_eff, labl="df_effe_size")
        return df_eff
    #
    df_eff = calculate_effect_sizes(df_modl)
    pass

    # $$$$
    # Exec 4 : Responder Analysis
    # $$$$
    def responder_analysis(df: pd.DataFrame, mcid_threshold: float) -> pd.DataFrame:
        """
        Identify individual responders based on MCID threshold.
        A responder is defined as Δ >= MCID (improvement direction).
        Computes responder proportions and Clopper–Pearson 95% CIs.
        """

        # Pivot to wide format
        wide = df.pivot(index='patient_id', columns='timepoint', values='VEINES_QOL_t')

        results = []
        for tp in ['T1', 'T2']:
            change = wide[tp] - wide['T0']
            responders = (change >= mcid_threshold).sum()
            total = change.notna().sum()
            ...
            results.append({
                ...
                "n_total": total,
                "proportion": responders / total,
                "percentage": 100 * responders / total,
                "ci_lower": 100 * ci_low,
                "ci_upper": 100 * ci_up,
        ...
        print_yes(df_resp, labl="df_resp_anal")
        return df_resp
    #
    df_resp_anal = responder_analysis(df_fram, mcid_thre)
    tab = df_resp_anal.copy()
    tab['n/N'] = tab.apply(lambda x: f"{x['n_responders']}/{x['n_total']}", axis=1)
    tab['Responder Rate (95% CI)'] = tab.apply(
        lambda x: f"{x['percentage']:.1f}% ({x['ci_lower']:.1f}–{x['ci_upper']:.1f}%)",
        axis=1
    )
    tab = tab[['Comparison', 'n/N', 'Responder Rate (95% CI)']]
    print(tab)

    # $$$$
    # Exec 5 : Combined Significance Matrix
    # $$$$
    def build_significance_matrix(df_popu_delt: pd.DataFrame,
                              df_resp_anal: pd.DataFrame,
                              mcid_threshold: float) -> pd.DataFrame:
        """
        Combine population-level change, responder rates, and MCID threshold
        into a single 'significance matrix'.
        """

        # Rename for clarity before merge
        pop = df_popu_delt.copy()
        pop.rename(columns={
        ...
        resp = df_resp_anal.copy()
        resp.rename(columns={
            "ci_lower": "resp_ci_lower",
            "ci_upper": "resp_ci_upper"
        }, inplace=True)

        # Merge on Comparison
        df_full = pop.merge(resp, on="Comparison", how="left")

        # Flags for significance
        df_full["statistically_significant"] = df_full["p_value"] < 0.05
        df_full["clinically_meaningful"] = df_full["mean_change"].abs() >= mcid_threshold
        df_full["meets_both_criteria"] = (
            df_full["statistically_significant"] & df_full["clinically_meaningful"]
        )

        print_yes(df_full, labl="df_full")
        return df_full
    
    df_full = build_significance_matrix(df_popu_delt, df_resp_anal, mcid_thre)

    # $$$$
    # Exec 6 : Summary
    # $$$$
    tab4 = df_full.copy()
    tab4['n/N'] = tab4.apply(lambda x: f"{x['n_responders']}/{x['n_total']}", axis=1)
    tab4['Responder Rate (95% CI)'] = tab4.apply(
        lambda x: f"{x['percentage']:.1f}% ({x['resp_ci_lower']:.1f}–{x['resp_ci_upper']:.1f}%)",
        axis=1
    )
    tab4['Stat. Sig (p<0.05)'] = tab4['statistically_significant'].map({True: 'Yes', False: 'No'})
    tab4['Clin. Sig (≥MCID)'] = tab4['clinically_meaningful'].map({True: 'Yes', False: 'No'})
    tab4['Combined Success'] = tab4['meets_both_criteria'].map({True: 'Yes', False: 'No'})
