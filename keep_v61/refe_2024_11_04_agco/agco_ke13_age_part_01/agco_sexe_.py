...
def agco_sexe():
    ...
    with open(jrnl_file_path, 'w') as file:
        ...
        df_resu_T = main(file_path, 'sexe', None)
        df_resu_M = main(file_path, 'sexe', 'M')
        df_resu_F = main(file_path, 'sexe', 'F')
        ...
        with pd.option_context('display.width', None, 'display.max_rows', None, 'display.max_columns', None, 'display.max_colwidth', None): 
            print(f"SEXE ages continus\n df_resu.size:{len(df_resu)} df_resu.type:{type(df_resu)}\n{df_resu}\n:{df_resu.index}\n:{df_resu.columns}")
            write(f"SEXE ages continus\n{df_resu}")
        xlsx = True
        if xlsx: 
            file_name = 'agco_sexe.xlsx'
