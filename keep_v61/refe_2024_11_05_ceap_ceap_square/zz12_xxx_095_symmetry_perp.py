...
# Symmetry
# -------------------------------
def symm_perp(what, df, indx_cate_list, colu_cate_list, indx_name, colu_name):

    # Prec 
    is_square = df.shape[0] == df.shape[1]
    if not is_square:
        print(f"\nData : {what}\n(perp) Symmetry : table must be square")
        write(f"\nData : {what}\n(perp) Symmetry : table must be square")
        return
    
    # Exec
    resu = SquareTable(df).symmetry()
    stat = resu.statistic
    pval = resu.pvalue
    ...
    print(f"\nData : {what}\n(perp) Symmetry : Stat:{stat_form} Pval:{pval_form}")
    write(f"\nData : {what}\n(perp) Symmetry : Stat:{stat_form} Pval:{pval_form}")  
    ...
    alpha = 0.05
    if pval < alpha:
        print(f"(perp) Symmetry : Reject the null hypothesis:\n{Ha}")
        write(f"(perp) Symmetry : Reject the null hypothesis:\n{Ha}")
    else:
        print(f"(perp) Symmetry : Fail to reject the null hypothesis:\n{H0}")
        write(f"(perp) Symmetry : Fail to reject the null hypothesis:\n{H0}")
    pass