...
# Pearson
# -------------------------------
def pear_perp(what, df, indx_cate_list, colu_cate_list, indx_name, colu_name):
        
    # Exec
    observed = df.values
    # Calculate association measures
    stat = pearson_c = contingency.association(observed, method="pearson")
    pear_intp = pearson_intp(stat)  
    ...
    print(f"\nData : {what}\n(perp) Pearson : Stat:{stat_form} Intp: asso(effect size):{pear_intp} Pval:{pval_form} Dof:{dof}")
    write(f"\nData : {what}\n(perp) Pearson : Stat:{stat_form} Intp: asso(effect size):{pear_intp} Pval:{pval_form} Dof:{dof}")
    print(f"(perp) Pearson : {com1_form}")
    write(f"(perp) Pearson : {com1_form}")
    print(f"(perp) Pearson : {com2_form}")
    write(f"(perp) Pearson : {com1_form}")
    ...
    if pval < alpha:
        print(f"(perp) Pearson : Reject the null hypothesis:\n{Ha}")
        write(f"(perp) Pearson : Reject the null hypothesis:\n{Ha}")
    else:
        print(f"(perp) Pearson : Fail to reject the null hypothesis:\n{H0}")
        write(f"(perp) Pearson : Fail to reject the null hypothesis:\n{H0}")
    pass